...
USE `vireo`;
DROP procedure IF EXISTS `remove_foreign_key_constraint`;
DELIMITER $$
USE `vireo`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `remove_foreign_key_constraint`(IN `dbName` varchar(255), IN `tableName` varchar(255), IN `columnName` varchar(255))
BEGIN
        DECLARE done BOOLEAN DEFAULT FALSE;
        DECLARE statement TEXT DEFAULT '';
        DECLARE cur1 CURSOR FOR SELECT CONCAT('ALTER TABLE \`', tableName, '\` DROP FOREIGN KEY ', t.constraint_name, ';')
...
END$$
DELIMITER ;
CALL remove_foreign_key_constraint('vireo', 'submission', 'embargoType_id');
DROP procedure IF EXISTS `remove_foreign_key_constraint`;
