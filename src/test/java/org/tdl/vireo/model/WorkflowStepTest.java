...
public class WorkflowStepTest extends AbstractEntityTest {
    ...
    public void testCreate() {
        ...
        assertEquals("Saved entity did not contain the field profile required value!", TEST_FIELD_PROFILE_REQUIRED, workflowStep.getFieldProfileByPredicate(fieldPredicate).getRequired());
    ...
    public void testCascade() {
        ...
        FieldProfile fieldProfile = fieldProfileRepo.create(fieldPredicate, TEST_FIELD_PROFILE_INPUT_TYPE, TEST_FIELD_PROFILE_REPEATABLE, TEST_FIELD_PROFILE_REQUIRED);
        FieldProfile detachableFieldProfile = fieldProfileRepo.create(detachableFieldPredicate, TEST_SEVERABLE_FIELD_PROFILE_INPUT_TYPE, TEST_SEVERABLE_FIELD_PROFILE_REPEATABLE, TEST_SEVERABLE_FIELD_PROFILE_REQUIRED);
        workflowStep.addFieldProfile(fieldProfile);
        ...
        assertEquals("Saved entity did not contain the field profile required value!", TEST_FIELD_PROFILE_REQUIRED, workflowStep.getFieldProfileByPredicate(fieldPredicate).getRequired());
        assertEquals("Saved entity did not contain the field profile input type!", TEST_FIELD_PROFILE_INPUT_TYPE, workflowStep.getFieldProfileByPredicate(fieldPredicate).getInputType());
        assertEquals("Saved entity did not contain the field profile repeatable value!", TEST_SEVERABLE_FIELD_PROFILE_REPEATABLE, workflowStep.getFieldProfileByPredicate(detachableFieldPredicate).getRepeatable());
        assertEquals("Saved entity did not contain the field profile required value!", TEST_SEVERABLE_FIELD_PROFILE_REQUIRED, workflowStep.getFieldProfileByPredicate(detachableFieldPredicate).getRequired());
        assertEquals("Saved entity did not contain the field profile input type!", TEST_SEVERABLE_FIELD_PROFILE_INPUT_TYPE, workflowStep.getFieldProfileByPredicate(detachableFieldPredicate).getInputType());

        // verify field predicates
        assertEquals("Saved entity did not contain the field profile field predicate value!", fieldPredicate, workflowStep.getFieldProfileByPredicate(fieldPredicate).getPredicate());
        assertEquals("Saved entity did not contain the field profile field predicate value!", detachableFieldPredicate, workflowStep.getFieldProfileByPredicate(detachableFieldPredicate).getPredicate());
