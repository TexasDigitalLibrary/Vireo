...
public class OrganizationTest extends AbstractEntityTest {
	...
	public void setUp() {
		...
		emailTemplate = emailTemplateRepo.create(TEST_EMAIL_TEMPLATE_NAME, TEST_EMAIL_TEMPLATE_SUBJECT,
	...
	public void testCreate() {
		...
		assertEquals("Saved entity did not contain the correct name!", TEST_PARENT_ORGANIZATION_NAME,
				parentOrganization.getName());
		assertEquals("Saved entity did not contain the correct category name!", parentCategory.getName(),
				parentOrganization.getCategory().getName());
		assertEquals("Saved entity did not contain the correct category level!", parentCategory.getLevel(),
				parentOrganization.getCategory().getLevel());
		assertEquals("Saved entity did not have the correct workflow name!", TEST_PARENT_WORKFLOW_NAME,
				parentOrganization.getWorkflow().getName());
		assertEquals("Saved entity did not have the correct workflow inheritability!",
				TEST_PARENT_WORKFLOW_INHERITABILITY, parentOrganization.getWorkflow().isInheritable());
		assertEquals("Saved entity did not have the emailWorkflow rule!", true,
				parentOrganization.getEmailWorkflowRules().contains(emailWorkflowRule));
	}
	...
	public void testFind() {
		organizationRepo.create(TEST_PARENT_ORGANIZATION_NAME, parentCategory);
		Organization parentOrganization = organizationRepo.findByNameAndCategory(TEST_PARENT_ORGANIZATION_NAME,
				parentCategory);
		assertNotEquals("Did not find entity!", null, parentOrganization);
		assertEquals("Found entity did not contain the correct name!", TEST_PARENT_ORGANIZATION_NAME,
				parentOrganization.getName());
	}
	...
	public void testCascade() {
		...
		Organization grandChildOrganization = organizationRepo.create(TEST_GRAND_CHILD_ORGANIZATION_NAME,
				grandChildCategory);
		Organization detachableParentOrganization = organizationRepo.create(TEST_DETACHABLE_PARENT_ORGANIZATION_NAME,
				parentCategory);
		Organization detachableChildOrganization = organizationRepo.create(TEST_DETACHABLE_CHILD_ORGANIZATION_NAME,
				childCategory);

		// create and add workflows to organizations
		parentOrganization
				.setWorkflow(workflowRepo.create(TEST_PARENT_WORKFLOW_NAME, TEST_PARENT_WORKFLOW_INHERITABILITY));
		childOrganization
				.setWorkflow(workflowRepo.create(TEST_CHILD_WORKFLOW_NAME, TEST_CHILD_WORKFLOW_INHERITABILITY));
		grandChildOrganization.setWorkflow(
				workflowRepo.create(TEST_GRAND_CHILD_WORKFLOW_NAME, TEST_GRAND_CHILD_WORKFLOW_INHERITABILITY));
		detachableParentOrganization.setWorkflow(workflowRepo.create(TEST_DETACHABLE_PARENT_WORKFLOW_NAME,
				TEST_DETACHABLE_PARENT_WORKFLOW_INHERITABILITY));
		detachableChildOrganization.setWorkflow(workflowRepo.create(TEST_DETACHABLE_CHILD_WORKFLOW_NAME,
				TEST_DETACHABLE_CHILD_WORKFLOW_INHERITABILITY));
		...
		assertEquals("The organization had incorrect number of parents!", 2,
				childOrganization.getParentOrganizations().size());

		// verify parent organization
		parentOrganization = (Organization) childOrganization.getParentOrganizations().toArray()[0]; // TODO:
																										// this
																										// sometimes
																										// fails
																										// because
																										...
																										// the
																										// same
		assertEquals("The parent organization did not have the correct name!", TEST_PARENT_ORGANIZATION_NAME,
				parentOrganization.getName());
		assertEquals("The parent organization category dit not have the correct name!", TEST_PARENT_CATEGORY_NAME,
				parentOrganization.getCategory().getName());
		assertEquals("The parent organization category dit not have the correct level!", TEST_PARENT_CATEGORY_LEVEL,
				parentOrganization.getCategory().getLevel());
		assertEquals("The parent organization has the wrong email workflow rule", true,
				parentOrganization.getEmailWorkflowRules().contains(emailWorkflowRule));

		// check number of child organizations of parent organization
		assertEquals("The parent organization had incorrect number of children!", 2,
				parentOrganization.getChildrenOrganizations().size());

		// verify child organization
		childOrganization = (Organization) parentOrganization.getChildrenOrganizations().toArray()[0];
		assertEquals("The parent's child organization did not have the correct name!", TEST_CHILD_ORGANIZATION_NAME,
				childOrganization.getName());
		assertEquals("The parent's child organization category did not have the correct Name!",
				TEST_CHILD_CATEGORY_NAME, childOrganization.getCategory().getName());
		assertEquals("The parent's child organization category did not have the correct Level!",
				TEST_CHILD_CATEGORY_LEVEL, childOrganization.getCategory().getLevel());

		// check number of child(grand child) organizations of child
		// organization
		assertEquals("The child organization had incorrect number of grand children!", 1,
				childOrganization.getChildrenOrganizations().size());

		// verify grand child organization
		grandChildOrganization = (Organization) childOrganization.getChildrenOrganizations().toArray()[0];
		assertEquals("The grand child organization did not have the correct name!", TEST_GRAND_CHILD_ORGANIZATION_NAME,
				grandChildOrganization.getName());
		assertEquals("The grand child organization category dit not have the correct Name!",
				TEST_GRAND_CHILD_CATEGORY_NAME, grandChildOrganization.getCategory().getName());
		assertEquals("The grand child organization category dit not have the correct Level!",
				TEST_GRAND_CHILD_CATEGORY_LEVEL, grandChildOrganization.getCategory().getLevel());

		// check the number of parent organizations of the grand child
		// organization
		assertEquals("The grand child organization had incorrect number of parents!", 1,
				grandChildOrganization.getParentOrganizations().size());
		...
		assertEquals("The parent organization had incorrect number of children!", 1,
				parentOrganization.getChildrenOrganizations().size());
		parentOrganization.removeEmailWorkflowRule(emailWorkflowRule);
		parentOrganization = organizationRepo.save(parentOrganization);
		assertEquals("The email workflow rule was not removed from the parent organization", 0,
				parentOrganization.getEmailWorkflowRules().size());
		...
		assertEquals("The child organization had incorrect number of parents!", 1,
				childOrganization.getParentOrganizations().size());
		...
		detachableParentOrganization = organizationRepo.save(detachableParentOrganization);
		childOrganization = organizationRepo.findOne(childOrganization.getId());
		assertEquals("The child organization had incorrect number of parents!", 2,
				childOrganization.getParentOrganizations().size());
		...
		assertEquals("The child organization had incorrect number of parents!", 0,
				childOrganization.getParentOrganizations().size());
		assertEquals("An organization category was deleted!", 3, organizationCategoryRepo.count());
		...
		assertEquals("The grand child organization had incorrect number of parents!", 0,
				grandChildOrganization.getParentOrganizations().size());
		assertEquals("An organization category was deleted!", 3, organizationCategoryRepo.count());
