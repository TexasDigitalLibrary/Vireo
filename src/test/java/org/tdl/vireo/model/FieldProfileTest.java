...
public class FieldProfileTest extends AbstractEntityTest {
	...
	public void testCreate() {
		...
		assertEquals("The field profile did not contain the correct perdicate value!", fieldPredicate,
				fieldProfile.getPredicate());
		assertEquals("The field predicate did not contain the correct value!", TEST_FIELD_PROFILE_REPEATABLE,
				fieldProfile.getRepeatable());
		assertEquals("The field predicate did not contain the correct value!", TEST_FIELD_PROFILE_REQUIRED,
				fieldProfile.getRequired());
	}

	@Override
	public void testDuplication() {
		fieldProfileRepo.create(fieldPredicate, TEST_FIELD_PROFILE_INPUT_TYPE, TEST_FIELD_PROFILE_REPEATABLE,
				TEST_FIELD_PROFILE_REQUIRED);
		try {
			fieldProfileRepo.create(fieldPredicate, TEST_FIELD_PROFILE_INPUT_TYPE, TEST_FIELD_PROFILE_REPEATABLE,
					TEST_FIELD_PROFILE_REQUIRED);
	...
	public void testCascade() {
		...
		ControlledVocabulary controlledVocabulary = controlledVocabularyRepo.create(TEST_CONTROLLED_VOCABULARY_NAME,
				language);
		ControlledVocabulary detachablecontrolledVocabulary = controlledVocabularyRepo
				.create(TEST_DETACHABLE_CONTROLLED_VOCABULARY_NAME, language);
		...
		assertEquals("The field profile did not contain the correct field gloss!", fieldGloss,
				(FieldGloss) fieldProfile.getFieldGlosses().toArray()[0]);
		assertEquals("The field profile did not contain the correct detachable field gloss!", detachableFieldGloss,
				(FieldGloss) fieldProfile.getFieldGlosses().toArray()[1]);
		assertEquals("The field profile did not contain the correct controlled vocabulary!", controlledVocabulary,
				fieldProfile.getControlledVocabularyByName(TEST_CONTROLLED_VOCABULARY_NAME));
		assertEquals("The field profile did not contain the correct detachable controlled vocabulary!",
				detachablecontrolledVocabulary,
				fieldProfile.getControlledVocabularyByName(TEST_DETACHABLE_CONTROLLED_VOCABULARY_NAME));

		// test detach detachable gloss
		fieldProfile.removeFieldGloss(detachableFieldGloss);
		fieldProfile = fieldProfileRepo.save(fieldProfile);
		assertEquals("The field profile had the incorrect number of glosses!", 1,
				fieldProfile.getFieldGlosses().size());

		// test detach detachable controlled vocabularies
		fieldProfile.removeControlledVocabulary(detachablecontrolledVocabulary);
		fieldProfile = fieldProfileRepo.save(fieldProfile);
		assertEquals("The field profile had the incorrect number of glosses!", 1,
				fieldProfile.getControlledVocabularies().size());
