...
public class InputTypeTest extends AbstractEntityTest {
    ...
    public void testCreate() {
        InputType inputType = inputTypeRepo.create(TEST_FIELD_PROFILE_INPUT_TYPE_NAME);
        assertEquals("The repository did not save the entity!", 1, inputTypeRepo.count());
        assertEquals("Saved entity did not contain the value!", TEST_FIELD_PROFILE_INPUT_TYPE_NAME, inputType.getName());
    }
    ...
    public void testDuplication() {
        inputTypeRepo.create(TEST_FIELD_PROFILE_INPUT_TYPE_NAME);
        try {
            inputTypeRepo.create(TEST_FIELD_PROFILE_INPUT_TYPE_NAME);
            assertTrue(false);
        } catch (Exception e) {
