...
public class CustomActionValueTest extends AbstractEntityTest {
	...
	public void testCreate() {
		CustomActionValue testCustomActionValue = customActionValueRepo.create(testSubmission,
				testCustomActionDefinition, TEST_CUSTOM_ACTION_VALUE);
		assertEquals("The custom action value was not created in the repository", 1, customActionValueRepo.count());
		assertEquals("Saved custom Action Value does not contain the correct submitter first name", TEST_USER_FIRSTNAME,
				testCustomActionValue.getSubmission().getSubmitter().getFirstName());
	...
	public void testFind() {
		CustomActionValue testCustomActionValue1 = customActionValueRepo.create(testSubmission,
				testCustomActionDefinition, TEST_CUSTOM_ACTION_VALUE);
		CustomActionValue testCustomActionValue2 = customActionValueRepo.findBySubmissionAndDefinition(testSubmission,
				testCustomActionDefinition);
		assertEquals("Found custom Action value for the correct submission", true,
				testCustomActionValue1.equals(testCustomActionValue2));
	}
